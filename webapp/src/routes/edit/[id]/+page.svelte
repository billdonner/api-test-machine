<script lang="ts">
	...
	import type { HttpMethod, RunDetail, DistributionStrategy, EndpointSpec } from '$lib/types';
	...
	onMount(async () => {
		...
		try {
			...
			// Pre-fill form with existing spec
			const spec = run.spec;
			name = spec.name || '';
			url = spec.url || '';
	...
	async function deleteRun() {
		if (!run) return;
		if (!confirm(`Delete this run of "${run.spec.name}"?`)) return;
...
<svelte:head>
	<title>Edit Test | API Test Machine</title>
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
	<div class="flex items-center justify-between">
		...
		{#if run}
			<button on:click={deleteRun} class="btn btn-danger text-sm">
				Delete Run
			</button>
		{/if}
	</div>
	...
	{:else if run}
		<div class="bg-blue-900/30 border border-blue-700/50 rounded-lg p-4 text-blue-300 text-sm">
			Editing will create a new test run with modified settings. The original run will remain unchanged.
		</div>
		...
		<form on:submit|preventDefault={submit} class="space-y-6">
			...
			<div class="flex justify-end gap-4">
				...
				<button type="submit" disabled={submitting} class="btn btn-primary">
					{submitting ? 'Starting...' : 'Run Modified Test'}
				</button>
			</div>
		</form>
	{/if}
